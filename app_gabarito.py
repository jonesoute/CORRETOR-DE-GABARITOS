...
# Função para detectar a orientação correta da imagem
def detectar_orientacao(imagem):
    ...
    if len(marcadores) < 2:
        return imagem  # Não rotaciona se não achar marcadores
...
# Função para gerar o PDF dos resultados
def gerar_pdf(df):
    pdf = FPDF()
    pdf.add_page()
...
uploaded_file = st.file_uploader("Gabarito base (em branco):", type=["jpg", "jpeg", "png"])

# Estado da sessão para armazenar coordenadas
...
# --- ETAPA 2: CORREÇÃO DO GABARITO RESPONDIDO ---
    ...
    if resp_file:
        ...
        for idx, (x, y) in enumerate(st.session_state.coords):
            ...
            if crop.size == 0:
                ...
                media = np.mean(crop)
                resultados.append("✅" if media < 127 else "❌")
